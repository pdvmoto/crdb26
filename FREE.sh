...
host read -t10 -p"Nomount. Control-C to cancel, Enter to continue..." abc
...
SET DEFAULT BIGFILE TABLESPACE    /*  set sizes based on finished database */
  ...
  SYSAUX 
    DATAFILE SIZE  800M AUTOEXTEND ON NEXT   101M MAXSIZE UNLIMITED
...
read -t30 -p"2_crdb_catalog done, control-C to stop..." abc
...
prompt no PDBs yet...
prompt .
prompt next are pdbs from 4_crdb_pdbs
prompt .

read -t30 -p"3_crdb_componetns done, control-C to stop..." abc
